...
resource "helm_release" "loadbalancer_controller" {
  ...
  set {  # Be careful this image is just for us-est-1 region. you can change it not using this region
  ...
  set {
    name  = "vpcId"
    value = data.aws_vpc.olivia-k8s_vpc.id
  }
  ...
  set {
    name  = "clusterName"
    value = data.aws_eks_cluster.olivia-k8s.name
  }
}