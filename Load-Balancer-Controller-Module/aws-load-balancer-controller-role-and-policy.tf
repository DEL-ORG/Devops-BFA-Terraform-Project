    ...
    Statement = [
      {
        ...
        Sid    = ""
        Principal = {
          Federated = join("/", ["arn:aws:iam::${data.aws_caller_identity.current.account_id}:oidc-provider", split("https://", data.aws_eks_cluster.revive.identity.0.oidc.0.issuer)[1]])
        }
        Condition = {
          StringEquals = {
            join(":", [split("https://", data.aws_eks_cluster.revive.identity.0.oidc.0.issuer)[1], "sub"]) : "system:serviceaccount:${var.aws-load-balancer-controller-ns}:${var.aws-load-balancer-controller-sa}"
          }
        }
      },
    ]
